...
export const GameTable: React.FC<GameTableProps> = ({
  ...
  useEffect(() => {
    ...
    if (currentCount > prevCount) {
      ...
      // Any player whose lastAction is 'PLAY' or who just placed a tile definitely did not pass
      const playedSeat = prevTurn !== null && prevTurn !== undefined
        ? Number(prevTurn)
        : ((Number(gameState.currentTurnSeat) - 1 + 4) % 4);
    ...
    // CASE 2: No tile placed, but a player passed! (Strictly plays ONLY pass double-knock sound)
      ...
      (currentCount === prevCount &&
        prevTurn !== null &&
        Number(prevTurn) !== Number(gameState.currentTurnSeat) &&
        gameState.players.find((p) => Number(p.seat) === Number(prevTurn))?.lastAction === 'PASS')
    ) {
      const candidateSeat = prevTurn !== null && Number(prevTurn) !== Number(gameState.currentTurnSeat)
        ? Number(prevTurn)
        : ((Number(gameState.currentTurnSeat) - 1 + 4) % 4);

      const candidatePlayer = gameState.players.find((p) => Number(p.seat) === candidateSeat);
      const passedSeat = candidatePlayer?.lastAction === 'PASS'
        ? candidateSeat
        : (gameState.players.find(
            (p) => Number(p.seat) !== Number(gameState.currentTurnSeat) && p.lastAction === 'PASS'
          )?.seat ?? candidateSeat);

      if (Number(passedSeat) !== Number(gameState.currentTurnSeat)) {
        setRecentlyPassedSeats((prev) => ({ ...prev, [Number(passedSeat)]: Date.now() }));
      }
  ...
  const renderBadgeOnly = (
    ...
    const isCurrentTurn =
      gameState.currentTurnSeat !== null &&
      gameState.currentTurnSeat !== undefined &&
      Number(gameState.currentTurnSeat) === Number(player.seat) &&
      gameState.status === 'PLAYING';
    const isBotPlayer = Boolean(
      player.isBot ||
      seatInfo?.isBot ||
    ...
    const isPassed =
      ...
      (!isMe || (gameState.myLegalMoves?.length ?? 0) === 0) &&
      (Date.now() - (recentlyPassedSeats[Number(player.seat)] || 0)) < 2200;
      ...
      <div style={{ position: 'relative', display: 'flex', flexDirection: orientation === 'column' ? 'column' : 'row', alignItems: 'center', gap: '6px' }}>
        ...
        <div style={{
          ...
          <div style={{ position: 'relative', flexShrink: 0 }}>
            ...
            {/* Compact Corner Mic Indicator for Mobile (Zero vertical space overhead) */}
            {!player.isBot && isMobile && (
              <div
                style={{
                  position: 'absolute',
                  bottom: '-2px',
                  ...
                  color: isPlayerSpeaking ? '#fff' : player.isVoiceMuted ? '#f87171' : '#9ca3af',
          ...
          {/* Active Speaking Mic Indicator for Desktop */}
          {!player.isBot && !isMobile && (
            <div
              style={{
          ...
          <div style={{ textAlign: orientation === 'column' ? 'center' : 'left', minWidth: 0, overflow: 'hidden' }}>
            <div style={{
              ...
              <span
                style={{
                  ...
                  maxWidth: isMobile ? (orientation === 'column' ? '54px' : '72px') : undefined,
              ...
              {isAway && (
                <span
                  ...
                  style={{
                    ...
                    fontSize: '0.62rem',
                    fontWeight: 900,
                    padding: isMobile && orientation === 'column' ? '0 2px' : '1px 5px',
                    borderRadius: '4px',
                ...
                >
                  {isMobile && orientation === 'column' ? '⚠️' : '⚠️ خارج اللعبة'}
                </span>
              )}
              {warnings === 1 && (
                <button
  ...
  const renderHiddenCards = (
    ...
    return (
      <div style={{
        display: 'flex',
        flexDirection: orientation,
        gap: orientation === 'column' ? 0 : (isLandscape ? '2px' : isMobile ? '2px' : '4px'),
        flexShrink: 0,
      }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh', maxHeight: '100dvh', width: '100vw', maxWidth: '100vw', background: 'radial-gradient(ellipse at 50% 30%, #e0f2fe 0%, #f0f9ff 45%, #e2e8f0 100%)', backgroundColor: '#f1f5f9', overflow: 'hidden' }}>
      {isJudge && (
      ...
      <div style={{
        ...
        borderBottom: '1.5px solid rgba(6, 182, 212, 0.25)',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: isLandscape ? '6px' : '10px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
            ...
            >
              <span style={{ fontSize: isLandscape ? '0.78rem' : '0.92rem', fontWeight: 900, color: '#080d1a', lineHeight: 1 }}>B</span>
            </div>
            <span className="arabic-font" style={{ fontSize: isLandscape ? '1.05rem' : '1.25rem', fontWeight: 900, color: '#d97706', letterSpacing: '0.5px' }}>بَفّة</span>
          </div>
        </div>
        ...
        <div
          ...
          style={{
            ...
            boxShadow: '0 6px 20px rgba(6, 182, 212, 0.2), 0 2px 6px rgba(0, 0, 0, 0.06)',
            userSelect: 'none',
          }}
        >
          ...
          {isObserver ? (
            <>
              <div style={{ display: 'flex', alignItems: 'center', gap: isLandscape ? '3px' : '5px' }}>
                <span className="arabic-font" style={{ fontSize: isLandscape ? '0.65rem' : '0.72rem', fontWeight: 800, color: '#64748b' }}>{getTeamLabel(1)}</span>
                <span style={{ fontSize: isLandscape ? '0.9rem' : '1.05rem', fontWeight: 900, color: '#0284c7' }}>{gameState.team1Score}</span>
              </div>
              <span style={{ color: 'rgba(0,0,0,0.25)', fontWeight: 900, fontSize: isLandscape ? '0.75rem' : '0.85rem' }}>:</span>
              <div style={{ display: 'flex', alignItems: 'center', gap: isLandscape ? '3px' : '5px' }}>
                <span style={{ fontSize: isLandscape ? '0.9rem' : '1.05rem', fontWeight: 900, color: '#e11d48' }}>{gameState.team2Score}</span>
                <span className="arabic-font" style={{ fontSize: isLandscape ? '0.65rem' : '0.72rem', fontWeight: 800, color: '#64748b' }}>{getTeamLabel(2)}</span>
              </div>
            </>
          ) : (
            <>
              <div style={{ display: 'flex', alignItems: 'center', gap: isLandscape ? '3px' : '5px' }}>
                <span className="arabic-font" style={{ fontSize: isLandscape ? '0.65rem' : '0.72rem', fontWeight: 800, color: '#64748b' }}>فريقك</span>
                <span style={{ fontSize: isLandscape ? '0.9rem' : '1.05rem', fontWeight: 900, color: '#0284c7' }}>{myTeamScore}</span>
              </div>
              <span style={{ color: 'rgba(0,0,0,0.25)', fontWeight: 900, fontSize: isLandscape ? '0.75rem' : '0.85rem' }}>:</span>
              <div style={{ display: 'flex', alignItems: 'center', gap: isLandscape ? '3px' : '5px' }}>
                <span style={{ fontSize: isLandscape ? '0.9rem' : '1.05rem', fontWeight: 900, color: '#e11d48' }}>{opponentScore}</span>
                <span className="arabic-font" style={{ fontSize: isLandscape ? '0.65rem' : '0.72rem', fontWeight: 800, color: '#64748b' }}>الخصم</span>
              </div>
            </>
          )}
          <div style={{
      ...
      <div style={{
        ...
        <div 
          ...
          background: 'radial-gradient(ellipse at 50% 50%, #22d3ee 0%, #06b6d4 50%, #0891b2 100%)', // Signature Bright Nile Turquoise Velvet Felt
          boxShadow: 'inset 0 0 50px rgba(6, 78, 99, 0.45), inset 0 0 15px rgba(255, 255, 255, 0.35), 0 12px 35px rgba(6, 182, 212, 0.25)',
          border: isLandscape
            ? '3px solid #fefcf3'
            : isMobile
            ? '4px solid #fefcf3'
            : '10px solid #fefcf3', // Warm Pearl Ivory Rim
          outline: isLandscape
            ? '2px solid #f59e0b'
            : isMobile
            ? '2.5px solid #f59e0b'
            : '3.5px solid #f59e0b', // Inlaid 24K Gold Bevel
          ...
          minHeight: 0,
            ...
            <div style={{ gridColumn: '1 / 2', gridRow: '2', display: 'flex', alignItems: 'center', justifyContent: 'center', maxHeight: '100%', overflow: 'visible' }}>
              {/* In RTL, gridColumn 1 is visually on the RIGHT. The Right opponent should sit here. */}
              {(isMobile || isLandscape) ? (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: isMobile ? '2px' : '4px', flexShrink: 0 }}>
                  {renderBadgeOnly(
                    relativePlayers.RIGHT,
                    getTeamLabel(relativePlayers.RIGHT?.team ?? 2),
                    'column',
            ...
            <div style={{ gridColumn: '3 / 4', gridRow: '2', display: 'flex', alignItems: 'center', justifyContent: 'center', maxHeight: '100%', overflow: 'visible' }}>
              ...
              >
                <div
                  style={{
                    ...
                    boxShadow: '0 0 30px rgba(255, 255, 255, 0.6), inset 0 0 25px rgba(255, 255, 255, 0.3)',
                    display: 'flex',
                    flexDirection: 'column',
                    alignItems: 'center',
                    justifyContent: 'center',
                ...
                <div style={{ 
                  ...
                  {placedTiles.length === 0 ? (
                    <div className="arabic-font animate-float" style={{ 
                      ...
                      color: '#0891b2', fontSize: '1.4rem', fontWeight: 900, whiteSpace: 'nowrap'
                    }}>
            ...
            {isRoundOver && (roundEndStage === 'DONE' || roundEndStage === 'IDLE' || roundEndStage === 'SCORE_PULSE') && (
              <div
                style={{
                  ...
                  borderRadius: '24px',
                  boxShadow: '0 8px 30px rgba(6, 182, 212, 0.25), 0 2px 8px rgba(0,0,0,0.1)',
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? '4px' : '8px', minWidth: 0 }}>
                  ...
                  <span className="arabic-font" style={{ color: '#0f172a', fontSize: isLandscape ? '0.72rem' : isMobile ? '0.76rem' : '0.9rem', fontWeight: 800, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                    {isMyTeamWinner
